...
//create new empty node(dabba)
        newnode= new node();  //newnode is a pointer variable storing the address of a node
        cout<<"\nEnter Data: ";    //with the help of new keyword we are dynamically creating a node structure
        cin>>newnode->info;
        newnode->next=beg;
        beg=newnode;
        break;
        ...
        cin>>newnode->info;
        newnode->next=NULL;
        // till now the node is created, now we have to insert it at the end
        if(beg==NULL){
            beg=newnode;
        }
        else{
            ptr=beg;   //we stored beg in ptr so that the value of beg does not change
...
void deletion(){
    ...
    switch(ch){
        case 1:
        beg=beg->next;   //here we broke the link of first and second node & assigned beg to second node
        break;
        ...
        ptr->next=ptr1->next;  //ptr1->next==NULL
        }
        break;
